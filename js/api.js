  ...
  $.get('https://api.myjson.com/bins/5bdb3', function(dataArray){
    ...
    dataArray.forEach(function(data,i){
      ...
      var monthlyAttendance = '.program-panel1.last-panel > div > .row > div > div.monthly-attendance' +i;
      // replace/insert data in corresponding div
      $(programName).text(data.Name);
      $(lowerProgramName).text(data.Name);
      $(totalMonthlySales).text(data.TotalMonthlySales);
      $(lowerMonthlySales).text(data.TotalMonthlySales);
